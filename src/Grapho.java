...
public class Grapho {
    ...
    private HashMap<Integer, Arista> edges;

    public Grapho(){
        this.vertices = new HashMap<String, Vertice>();
        this.edges = new HashMap<Integer, Arista>();
    }
     ...
     */
    public Grapho(ArrayList<Vertice> vertices){
        this.vertices = new HashMap<String, Vertice>();
        this.edges = new HashMap<Integer, Arista>();
     ...
     * exists in the Graph.
     *
     * @param one The first Vertice to add
     * @param two The second Vertice to add
     * @return true iff no Edge relating one and two exists in the Graph
     */
    public boolean addArista(Vertice one, Vertice two){
     ...
     * exists in the Graph.
     *
     * @param one The first Vertice of the Edge
     * @param two The second Vertice of the Edge
    ...
    public boolean addEdge(Vertice one, Vertice two, int weight){
        ...
        Arista e = new Arista(one, two, weight);
        if(edges.containsKey(e.hashCode())){
            return false;
        }

        //and that the Edge isn't already incident to one of the vertices
        else if(one.estadoConexion(e) || two.estadoConexion(e)){
            return false;
        }
     ...
     */
    public boolean containsEdge(Arista e){
     ...
     * including as each Vertice's incidence conexionhood.
     *
     * @param e The Edge to remove from the Graph
     * @return Edge The Edge removed from the Graph
     */
    public Arista removeEdge(Arista e){
        e.getOne().removeConexion(e);
        e.getTwo().removeConexion(e);
    ...
    public boolean containsVertex(Vertice vertex){
        return this.vertices.get(vertex.getNombre()) != null;
    }
     ...
     */
    public boolean addVertice(Vertice vertex, boolean overwriteExisting){
        Vertice current = this.vertices.get(vertex.getNombre());
        if(current != null){
            if(!overwriteExisting){
                return false;
            }

            while(current.getConexionCount() > 0){
                this.removeEdge(current.getConexion(0));
            }
        }
    ...
    public Vertice removeVertex(String label){
        ...
        while(v.getConexionCount() > 0){
            this.removeEdge(v.getConexion((0)));
        }
     ...
     */
    public Set<Arista> getEdges(){
        return new HashSet<Arista>(this.edges.values());
    }
